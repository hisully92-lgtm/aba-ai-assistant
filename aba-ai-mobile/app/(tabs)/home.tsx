...
export default function HomeScreen() {
  ...
  async function selectClientForClockIn(client: Client) {
    ...
    const { data } = await supabase.from("client_locations").select("*").eq("client_id", client.id).eq("company_id", companyId).order("is_primary", { ascending: false });
                ...
                <View style={styles.geofenceBox}>
                  {geofenceStatus === "checking" && (
                    <>
                      ...
                      <Text style={styles.geofenceText}>Checking your location...</Text>
                    </>
                  )}
                  {geofenceStatus === "inside" && (
                    <>
                      ...
                      <Text style={styles.geofenceDist}>{geofenceDistance}m from address</Text>
                    </>
                  )}
                  {geofenceStatus === "outside" && (
                    <>
                      ...
                      <Text style={styles.geofenceDist}>Must be within 300m to clock in</Text>
                    </>
                  )}
                  {geofenceStatus === "error" && (
                    <>
                      <Text style={styles.geofenceEmoji}>❌</Text>
                      <Text style={[styles.geofenceText, { color: "#dc2626" }]}>Location access denied</Text>
                    </>
                  )}
                </View>
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 8 }}>
                      {ADJUSTMENT_REASONS.map(r => (
                        <TouchableOpacity key={r}
                          style={[styles.reasonChip, startReason === r && styles.reasonChipActive]}
              ...
              <>
                ...
                <View style={styles.confirmBox}>
                  <View style={styles.confirmRow}>
                    ...
                    <Text style={styles.confirmValue}>{selectedClient?.full_name}</Text>
                  </View>
                  <View style={styles.confirmRow}>
                    <Text style={styles.confirmLabel}>Location</Text>
                    <Text style={styles.confirmValue}>{selectedLocation?.name}</Text>
                  </View>
                  <View style={styles.confirmRow}>
                    <Text style={styles.confirmLabel}>Address</Text>
                    <Text style={styles.confirmValue}>{selectedLocation?.address}, {selectedLocation?.city}</Text>
                  </View>
                  <View style={styles.confirmRow}>
                    <Text style={styles.confirmLabel}>Start Time</Text>
                    <Text style={styles.confirmValue}>{startTime} {startAdjusted ? "⚠️ Adjusted" : "✓"}</Text>
                  </View>
                  {startAdjusted && startReason && (
                    <View style={styles.confirmRow}>
                      <Text style={styles.confirmLabel}>Reason</Text>
                      <Text style={styles.confirmValue}>{startReason}</Text>
                    </View>
                  )}
                  <View style={styles.confirmRow}>
                ...
                <TouchableOpacity style={[styles.modalPrimary, clockingIn && { opacity: 0.6 }]}
                  onPress={confirmClockIn} disabled={clockingIn}>
      ...
      <Modal visible={showClockOutModal} animationType="slide" transparent onRequestClose={() => setShowClockOutModal(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalPanel}>
            ...
            {endAdjusted && (
              <View style={{ marginTop: 12 }}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 8 }}>
                  {ADJUSTMENT_REASONS.map(r => (
                    <TouchableOpacity key={r}
                      style={[styles.reasonChip, endReason === r && styles.reasonChipActive]}
            ...
            <TouchableOpacity style={[styles.clockOutConfirmBtn, clockingOut && { opacity: 0.6 }]}
              onPress={confirmClockOut} disabled={clockingOut}>
