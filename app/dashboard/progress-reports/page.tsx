...
export default function ProgressReportsPage() {
  ...
  if (!win) return;
  win.document.write(`
    <html><head><title>Progress Report - ${client?.full_name ?? "Client"}</title>
    <style>
      * { box-sizing: border-box; margin: 0; padding: 0; }
      body { font-family: Arial, sans-serif; padding: 48px; max-width: 800px; margin: 0 auto; color: #111827; font-size: 14px; line-height: 1.6; }
      ...
      .ai-badge { display: inline-block; background: #f3e8ff; color: #7c3aed; padding: 2px 10px; border-radius: 999px; font-size: 12px; margin-top: 8px; }
      .section { margin-bottom: 28px; }
      .section h2 { color: #1e40af; font-size: 15px; font-weight: bold; text-transform: uppercase; letter-spacing: 0.05em; border-bottom: 1px solid #e5e7eb; padding-bottom: 6px; margin-bottom: 12px; }
      .section p { color: #374151; white-space: pre-wrap; }
      .footer { margin-top: 48px; padding-top: 16px; border-top: 1px solid #e5e7eb; display: grid; grid-template-columns: 1fr 1fr; gap: 32px; }
      .sig-line { border-top: 1px solid #374151; padding-top: 4px; font-size: 12px; color: #6b7280; margin-top: 32px; }
      @media print { body { padding: 32px; } }
    </style>
    </head><body>
    <div class="header">
      <h1>ABA Progress Report</h1>
      <div class="meta">
        <div class="meta-item"><strong>Client:</strong> ${client?.full_name ?? "Unknown"}</div>
        <div class="meta-item"><strong>Report Period:</strong> ${report.report_period}</div>
        <div class="meta-item"><strong>Date Generated:</strong> ${new Date(report.created_at).toLocaleDateString()}</div>
        <div class="meta-item"><strong>Generated By:</strong> ABA AI Platform</div>
      </div>
      ${report.generated_by_ai ? '<div class="ai-badge">AI-Assisted Report</div>' : ""}
    </div>
    ${report.goals_addressed ? `
    <div class="section">
      <h2>Goals Addressed</h2>
      <p>${report.goals_addressed.replace(/\n/g, "<br>")}</p>
    </div>` : ""}
    <div class="section">
    ...
    <div class="footer">
      <div>
        <div class="sig-line">Clinician Signature &amp; Credentials</div>
        <div class="sig-line" style="margin-top:16px">Date</div>
      </div>
      <div>
        <div class="sig-line">Supervisor Signature &amp; Credentials</div>
        <div class="sig-line" style="margin-top:16px">Date</div>
      </div>
    </div>
    </body></html>
