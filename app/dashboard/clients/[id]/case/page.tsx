...
export default function CaseDrilldownPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    async function init() {
      ...
      await Promise.all([loadClient(), loadRisk(), loadSessions(), loadAlerts()]);
      setLoading(false);
    }
    init();
    ...
    subscriptionRef.current = supabase
      ...
      .on("postgres_changes", {
        event: "*", schema: "public", table: "client_risk",
  ...
  async function loadRisk() {
    ...
    setRisk(
      data
        ...
        : { risk_score: null, forecast_score: null, risk_level: "unknown", updated_at: null }
    );
  }
  ...
  if (loading) return <div className="p-6 text-gray-400">Loading case...</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      <Section title="Client Overview">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-gray-600">
          {client.date_of_birth && (
            <p><span className="font-medium">DOB:</span> {new Date(client.date_of_birth).toLocaleDateString()}</p>
          )}
          {client.guardian_name && (
            <p><span className="font-medium">Guardian:</span> {client.guardian_name}</p>
          )}
          {risk?.updated_at && (
            <p><span className="font-medium">Risk Updated:</span> {new Date(risk.updated_at).toLocaleDateString()}</p>
          )}
          <p>
            <span className="font-medium">Open Alerts:</span>{" "}
            <span className={openAlerts > 0 ? "text-red-600 font-semibold" : ""}>{openAlerts}</span>
          </p>
        </div>
      </Section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="font-semibold">{risk?.risk_score != null ? `${risk.risk_score}%` : "N/A"}</span>
              </div>
            ...
            <div>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="font-semibold">{risk?.forecast_score != null ? `${risk.forecast_score}%` : "N/A"}</span>
              </div>
      ...
      <Section title="AI Clinical Summary">
        ...
        <Button onClick={handleGenerateSummary} loading={generating} variant="secondary">
          Generate AI Summary
        </Button>
      </Section>
          ...
          <div className="space-y-2">
            {sessions.map((s) => (
              <div key={s.id}
                ...
                <div>
                  <p className="font-medium text-gray-700">{new Date(s.created_at).toLocaleDateString()}</p>
                  {s.notes && <p className="text-gray-400 text-xs mt-0.5 line-clamp-1">{s.notes}</p>}
                </div>
