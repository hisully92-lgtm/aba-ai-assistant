...
export default function ABAGraphsPage() {
  ...
  useEffect(() => { initClients(); }, []); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  async function addPhase() {
    ...
    const { data } = await supabase.from("graph_phases").insert([{
      client_id: selectedClient, phase_name: phaseName, start_date: phaseDate, phase_color: phaseColor, created_by: user.id,
    }]).select().single();
  ...
  async function addGoal() {
    ...
    const { data } = await supabase.from("graph_goals").insert([{
      client_id: selectedClient, target_name: goalName, goal_value: goalValue, color: goalColor, created_by: user.id,
    }]).select().single();
  ...
  return (
    <div className="space-y-6">
      ...
      {!analyticsGate.allowed && (
        <>
          <UpgradePrompt
            reason={`ABA clinical graphs require the Professional plan or higher. You are on the ${planName} plan.`}
            upgradeTo={analyticsGate.upgradeTo}
            feature="ABA Clinical Graphs"
          />
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { icon: "📈", title: "Line Graph", desc: "Standard equal-interval ABA data graph" },
              { icon: "📉", title: "Celeration Chart", desc: "Semi-log Standard Celeration Chart" },
              { icon: "🔵", title: "Scatterplot", desc: "Identify behavior patterns by time of day" },
            ].map(item => (
        ...
        <>
          ...
          {selectedClient && !loading && hasData && (
            <>
              ...
              {/* LINE GRAPH */}
              {selectedGraph === "line" && (
                <Section title={`📈 Line Graph — ${selectedClientObj?.full_name}`}>
                  <p className="text-xs text-gray-400 mb-4">Standard ABA equal-interval graph.</p>
                  <ResponsiveContainer width="100%" height={360}>
                    <LineChart data={chartData} margin={{ top: 10, right: 30, left: 10, bottom: 30 }}>
                      ...
                      <Legend />
                      {phases.map(phase => (
                        <ReferenceLine key={phase.id} x={phase.start_date} stroke={phase.phase_color} strokeWidth={2} strokeDasharray="6 3"
                          label={{ value: phase.phase_name, position: "top", fontSize: 10, fill: phase.phase_color }} />
                      ))}
                      {goals.map(goal => (
                        <ReferenceLine key={goal.id} y={goal.goal_value} stroke={goal.color} strokeWidth={2} strokeDasharray="8 4"
                          label={{ value: goal.target_name, position: "right", fontSize: 10, fill: goal.color }} />
                      ))}
                      {displayTargets.map((target, i) => (
                        <Line key={target} type="monotone" dataKey={target} stroke={COLORS[i % COLORS.length]}
                          strokeWidth={2.5} dot={{ r: 4, fill: COLORS[i % COLORS.length] }} activeDot={{ r: 6 }} connectNulls />
                      ))}
                    </LineChart>
                  </ResponsiveContainer>
                </Section>
              ...
              {selectedGraph === "celeration" && (
                <Section title={`📉 Celeration Graph — ${selectedClientObj?.full_name}`}>
                  <p className="text-xs text-gray-400 mb-4">Semi-logarithmic scale. Slope = rate of behavior change.</p>
                  <ResponsiveContainer width="100%" height={360}>
                    <LineChart data={celerationData} margin={{ top: 10, right: 30, left: 20, bottom: 30 }}>
                      ...
                      <XAxis dataKey="session" type="number" scale="linear" tick={{ fontSize: 10 }}>
                        <Label value="Session Number" offset={-10} position="insideBottom" fontSize={11} />
                      </XAxis>
                      ...
                      {phases.map(phase => (
                        <ReferenceLine key={phase.id} x={phases.indexOf(phase) + 1} stroke={phase.phase_color} strokeWidth={2} strokeDasharray="6 3"
                          label={{ value: phase.phase_name, position: "top", fontSize: 10, fill: phase.phase_color }} />
                      ))}
                      {goals.map(goal => (
                        <ReferenceLine key={goal.id} y={goal.goal_value} stroke={goal.color} strokeWidth={2} strokeDasharray="8 4"
              ...
              {selectedGraph === "bar" && (
                <Section title={`📊 Bar Graph — ${selectedClientObj?.full_name}`}>
                  <p className="text-xs text-gray-400 mb-4">Frequency or duration per session.</p>
                  <ResponsiveContainer width="100%" height={360}>
              ...
              {selectedGraph === "cumulative" && (
                <Section title={`📋 Cumulative Record — ${selectedClientObj?.full_name}`}>
                  <p className="text-xs text-gray-400 mb-4">Running total of responses. Slope = response rate.</p>
                  <ResponsiveContainer width="100%" height={360}>
                    <LineChart data={cumulativeData} margin={{ top: 10, right: 30, left: 10, bottom: 30 }}>
                      ...
                      <Line type="monotone" dataKey="cumulative" stroke="#2563eb" strokeWidth={3}
                        dot={{ r: 3, fill: "#2563eb" }} activeDot={{ r: 6 }} name="Cumulative Responses" />
                    </LineChart>
                  </ResponsiveContainer>
                </Section>
              )}
              ...
              {selectedGraph === "scatterplot" && (
                <Section title={`🔵 Scatterplot — ${selectedClientObj?.full_name}`}>
                  <p className="text-xs text-gray-400 mb-4">Time of day vs behavior frequency. Clusters reveal antecedent patterns.</p>
                  <ResponsiveContainer width="100%" height={360}>
                    <ScatterChart margin={{ top: 10, right: 30, left: 10, bottom: 30 }}>
                      ...
                      {displayTargets.map((target, i) => (
                        <Scatter key={target} name={target}
                      ...
                      {chartData.map((row: any, i: number) => {
                        ...
                        return (
                          <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                            ...
                            {displayTargets.map(t => (
                              <td key={t} className="px-3 py-2 border border-gray-100 text-gray-600">{row[t] ?? "—"}</td>
                            ))}
